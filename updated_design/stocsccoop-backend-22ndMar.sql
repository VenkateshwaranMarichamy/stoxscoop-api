...
CREATE TYPE stoxscoop.event_type_enum AS ENUM (
    'corporate_action',
    'disclosure',
    'insider',
    'business',
...
CREATE TYPE stoxscoop.signal_type_enum AS ENUM (
    'bullish',
    'bearish',
    'neutral',
    'mixed'
...
CREATE TYPE stoxscoop.priority_enum AS ENUM (
    'low',
    'medium',
    'high'
);

CREATE TYPE stoxscoop.sentiment_enum AS ENUM (
    'positive',
    'negative',
    'neutral'
);

CREATE TYPE stoxscoop.ingestion_source_enum AS ENUM (
    'manual',
    'rss',
    'api',
    'scraper',
...
CREATE TYPE stoxscoop.contract_type_enum AS ENUM (
    'mou',
    'loi',
    'confirmed',
    'partnership',
...
CREATE TYPE stoxscoop.transaction_type_enum AS ENUM (
    'buy',
    'sell',
    'increase',
    'decrease',
...
CREATE TYPE stoxscoop.transaction_mode_enum AS ENUM (
    'bulk_deal',
    'block_deal',
    'open_market',
    'off_market'
...
CREATE TYPE stoxscoop.investor_category_enum AS ENUM (
    'superinvestor',
    'fii',
    'dii',
    'hni',
...
CREATE TYPE stoxscoop.rating_agency_enum AS ENUM (
    'crisil',
    'icra',
    'care',
    'india_ratings',
...
CREATE TYPE stoxscoop.outcome_enum AS ENUM (
    'favorable',
    'adverse',
    'pending',
    'settled'
...
CREATE TYPE stoxscoop.beat_miss_enum AS ENUM (
    'beat',
    'miss',
    'inline'
);
...
CREATE TABLE stoxscoop.event_subtypes (
...
CREATE TABLE stoxscoop.event_batches (
...
CREATE TABLE stoxscoop.events (
    id                          SERIAL                NOT NULL PRIMARY KEY,
    stock_id                    INTEGER               NOT NULL,
    batch_id                    INTEGER               REFERENCES stoxscoop.event_batches(id) ON DELETE SET NULL,
    ...
    CONSTRAINT fk_events_subtype
        ...
        REFERENCES stoxscoop.event_subtypes (event_type, subtype_code),
...
CREATE TABLE stoxscoop.corporate_action_details (
    id                  SERIAL          NOT NULL PRIMARY KEY,
    event_id            INTEGER         NOT NULL UNIQUE REFERENCES stoxscoop.events(id) ON DELETE CASCADE,
    record_date         DATE,
    effective_date      DATE,
...
COMMENT ON TABLE stoxscoop.corporate_action_details IS
    'Covers: dividend, bonus, split, rights_issue, buyback, '
    'merger, demerger, open_offer.';


-- 4b. Disclosures
CREATE TABLE stoxscoop.disclosure_details (
    id                  SERIAL                  NOT NULL PRIMARY KEY,
    event_id            INTEGER                 NOT NULL UNIQUE REFERENCES stoxscoop.events(id) ON DELETE CASCADE,
...
-- 4c. Insider activity
CREATE TABLE stoxscoop.insider_details (
    id                      SERIAL                      NOT NULL PRIMARY KEY,
    event_id                INTEGER                     NOT NULL UNIQUE REFERENCES stoxscoop.events(id) ON DELETE CASCADE,
...
-- 4d. Business events
CREATE TABLE stoxscoop.business_event_details (
    id                  SERIAL                  NOT NULL PRIMARY KEY,
    event_id            INTEGER                 NOT NULL UNIQUE REFERENCES stoxscoop.events(id) ON DELETE CASCADE,
...
-- 4e. Governance
CREATE TABLE stoxscoop.governance_details (
    id                  SERIAL          NOT NULL PRIMARY KEY,
    event_id            INTEGER         NOT NULL UNIQUE REFERENCES stoxscoop.events(id) ON DELETE CASCADE,
...
-- 4f. Credit ratings
CREATE TABLE stoxscoop.credit_rating_details (
    id                  SERIAL                  NOT NULL PRIMARY KEY,
    event_id            INTEGER                 NOT NULL UNIQUE REFERENCES stoxscoop.events(id) ON DELETE CASCADE,
...
CREATE TABLE stoxscoop.financial_result_details (
    id                  SERIAL              NOT NULL PRIMARY KEY,
    event_id            INTEGER             NOT NULL UNIQUE REFERENCES stoxscoop.events(id) ON DELETE CASCADE,
...
-- 4h. Fundraising
CREATE TABLE stoxscoop.fundraising_details (
    id                  SERIAL          NOT NULL PRIMARY KEY,
    event_id            INTEGER         NOT NULL UNIQUE REFERENCES stoxscoop.events(id) ON DELETE CASCADE,
...
-- 4i. Legal & litigation
CREATE TABLE stoxscoop.legal_details (
    id                      SERIAL              NOT NULL PRIMARY KEY,
    event_id                INTEGER             NOT NULL UNIQUE REFERENCES stoxscoop.events(id) ON DELETE CASCADE,
    forum                   VARCHAR(100),
...
CREATE INDEX idx_events_stock_date          ON stoxscoop.events (stock_id, event_date DESC);

-- Classification screener
CREATE INDEX idx_events_type_subtype        ON stoxscoop.events (event_type, event_subtype);

-- Signal / priority / sentiment filters
CREATE INDEX idx_events_signal              ON stoxscoop.events (signal_type);
CREATE INDEX idx_events_priority            ON stoxscoop.events (priority);
CREATE INDEX idx_events_sentiment           ON stoxscoop.events (sentiment);

-- AI quality filter — WHERE confidence_score >= 0.75
CREATE INDEX idx_events_confidence          ON stoxscoop.events (confidence_score);

-- Tags array containment — WHERE tags @> '{bulk_deal}'
CREATE INDEX idx_events_tags                ON stoxscoop.events USING GIN (tags);

-- Batch + audit
CREATE INDEX idx_events_batch               ON stoxscoop.events (batch_id, created_at DESC);
CREATE INDEX idx_events_ingestion_source    ON stoxscoop.events (ingestion_source, created_at DESC);
CREATE INDEX idx_active_events ON stoxscoop.events(event_date) WHERE is_active = TRUE;

-- Date range
CREATE INDEX idx_events_event_date          ON stoxscoop.events (event_date DESC);


-- Detail table filters
CREATE INDEX idx_disclosure_category        ON stoxscoop.disclosure_details (investor_category);
CREATE INDEX idx_disclosure_investor        ON stoxscoop.disclosure_details (investor_name);
CREATE INDEX idx_insider_person             ON stoxscoop.insider_details (person_name);
CREATE INDEX idx_business_contract_type     ON stoxscoop.business_event_details (contract_type);
CREATE INDEX idx_credit_agency              ON stoxscoop.credit_rating_details (agency);
CREATE INDEX idx_legal_outcome              ON stoxscoop.legal_details (outcome);
...
INSERT INTO stoxscoop.event_subtypes
